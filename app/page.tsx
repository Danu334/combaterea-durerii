...
function TicketForm({ ticketType, ticketName, index, value, onChange, isMobile }: {
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #eee', borderRadius: '12px', padding: isMobile ? '1.25rem' : '2rem', marginBottom: '1.5rem' }}>
...
function MobileOrderSummary({ cart, forms, total, formatMDL, getHandzoneLabel, loading }: {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  cart: any[]; forms: AnyForm[]; total: number
  formatMDL: (n: number) => string
  getHandzoneLabel: (v: HandzoneOption) => string | null
  loading: boolean
}) {
  const [open, setOpen] = useState(false)

  return (
    <div style={{ background: '#fff', border: '1px solid #eee', borderRadius: '12px', marginBottom: '1.5rem', overflow: 'hidden' }}>
      {/* Toggle header */}
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
      ...
      >
        <span style={{ fontSize: '14px', fontWeight: '600', color: '#1a3a6b' }}>
          {open ? 'Ascunde sumar' : 'Arată sumar comandă'}
        </span>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ transform: open ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s' }}>
            <path d="M3 6l5 5 5-5" stroke="#1a3a6b" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </div>
      </button>
      ...
      {open && (
        <div style={{ padding: '0 16px 16px', borderTop: '1px solid #f0f0f0' }}>
          ...
          <div style={{ borderTop: '1px solid #eee', marginTop: '8px', paddingTop: '12px', display: 'flex', justifyContent: 'space-between' }}>
...
export default function CartPage() {
  ...
  return (
    <div style={{ fontFamily: '"DM Sans", sans-serif', minHeight: '100vh', background: '#f7f8fa' }}>
      ...
      <main style={{ maxWidth: '1100px', margin: '0 auto', padding: isMobile ? '1.25rem 1rem 6rem' : '2rem 2rem 5rem' }}>
        ...
        <form onSubmit={handleSubmit}>
          {isMobile ? (
            /* ── MOBILE LAYOUT ───────────────────────────────────────────── */
            <div>
              {/* Collapsible summary at top on mobile */}
              <MobileOrderSummary
                cart={cart} forms={forms} total={total}
                formatMDL={formatMDL} getHandzoneLabel={getHandzoneLabel}
                loading={loading}
              />
              ...
              {cart.map((item, i) => (
                <div key={`${item.id}-${i}`} style={{ position: 'relative' }}>
                  ...
                  <button type="button"
                    ...
                    style={{ position: 'absolute', top: '1.25rem', right: '1rem', background: 'none', border: 'none', cursor: 'pointer', color: '#bbb', fontSize: '18px', lineHeight: 1, padding: '4px' }}
                    title="Șterge">✕</button>
                </div>
              ))}

              {/* Sticky bottom bar on mobile */}
              <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#fff', borderTop: '1px solid #eee', padding: '12px 16px', zIndex: 100, display: 'flex', alignItems: 'center', gap: '12px', boxShadow: '0 -2px 12px rgba(0,0,0,0.07)' }}>
                <div style={{ flex: 1 }}>
                  <p style={{ margin: 0, fontSize: '11px', color: '#aaa' }}>Total</p>
                  <p style={{ margin: 0, fontSize: '16px', fontWeight: '700', color: '#1a3a6b' }}>{formatMDL(total)}</p>
                </div>
                <button
                  type="submit"
                  disabled={loading}
                  style={{ padding: '13px 22px', background: loading ? '#888' : '#1a3a6b', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}
                >
